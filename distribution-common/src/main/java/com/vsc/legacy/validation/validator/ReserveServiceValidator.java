...
public class ReserveServiceValidator {
    ...
    public com.vsc.legacy.model.reserve.cancel.response.Errors validate(VSCCancelRQ cancelRQ) {

        Set<ConstraintViolation<VSCCancelRQ>> violations = validator.validate(cancelRQ);
        com.vsc.legacy.model.reserve.cancel.response.Errors errors = null;
        if(!CollectionUtils.isEmpty(violations)) {
            errors = new com.vsc.legacy.model.reserve.cancel.response.Errors();
            for(ConstraintViolation<VSCCancelRQ> violation : violations) {
                com.vsc.legacy.model.reserve.cancel.response.Error error = getCancelError(violation);
                if (error != null) {
    ...
    private com.vsc.legacy.model.reserve.cancel.response.Error getCancelError(ConstraintViolation violation) {
        com.vsc.legacy.model.reserve.cancel.response.Error error = null;
        if (!StringUtils.isEmpty(violation.getMessage())) {
            error = new com.vsc.legacy.model.reserve.cancel.response.Error();
            error.setContent(violation.getMessage());
            String errorCode = violation.getMessageTemplate();
            errorCode = StringUtils.trimLeadingCharacter(errorCode,'{');
            errorCode = StringUtils.trimTrailingCharacter(errorCode, '}');
    ...
    public com.vsc.legacy.model.reserve.details.response.Errors validate(VSCRetrieveResDetailsRQ retrieveResDetailsRQ) {

        Set<ConstraintViolation<VSCRetrieveResDetailsRQ>> violations = validator.validate(retrieveResDetailsRQ);
        com.vsc.legacy.model.reserve.details.response.Errors errors = null;
        if(!CollectionUtils.isEmpty(violations)) {
            ...
            for(ConstraintViolation<VSCRetrieveResDetailsRQ> violation : violations) {
                com.vsc.legacy.model.reserve.details.response.Error error = getRetrieveReservationError(violation);
                if (error != null) {
    ...
    private com.vsc.legacy.model.reserve.details.response.Error getRetrieveReservationError(ConstraintViolation violation) {
    	com.vsc.legacy.model.reserve.details.response.Error error = null;
        if (!StringUtils.isEmpty(violation.getMessage())) {
            error = new com.vsc.legacy.model.reserve.details.response.Error();
            error.setContent(violation.getMessage());
            String errorCode = violation.getMessageTemplate();
            errorCode = StringUtils.trimLeadingCharacter(errorCode,'{');
            errorCode = StringUtils.trimTrailingCharacter(errorCode, '}');
