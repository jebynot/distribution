...
public class PropertySearchValidator {
    ...
    public com.vsc.legacy.model.property.summary.response.Errors validate(VSCPropertyListRQ propertyListRQ) {

        Set<ConstraintViolation<VSCPropertyListRQ>> violations = validator.validate(propertyListRQ);
        com.vsc.legacy.model.property.summary.response.Errors errors = null;
        if(!CollectionUtils.isEmpty(violations)) {
            ...
            for(ConstraintViolation<VSCPropertyListRQ> violation : violations) {
                com.vsc.legacy.model.property.summary.response.Error error = getPropertyListError(violation);
                if (error != null) {
        ...
        private com.vsc.legacy.model.property.summary.response.Error getPropertyListError(ConstraintViolation violation) {
            com.vsc.legacy.model.property.summary.response.Error error = null;
            if (!StringUtils.isEmpty(violation.getMessage())) {
                error = new com.vsc.legacy.model.property.summary.response.Error();
                error.setContent(violation.getMessage());
                String errorCode = violation.getMessageTemplate();
                errorCode = StringUtils.trimLeadingCharacter(errorCode,'{');
                errorCode = StringUtils.trimTrailingCharacter(errorCode, '}');
    ...
    public com.vsc.legacy.model.availability.response.Errors validate(VSCAvailRQ availRQ) {

        Set<ConstraintViolation<VSCAvailRQ>> violations = validator.validate(availRQ);
        com.vsc.legacy.model.availability.response.Errors errors = null;
        if(!CollectionUtils.isEmpty(violations)) {
            errors = new com.vsc.legacy.model.availability.response.Errors();
            for(ConstraintViolation<VSCAvailRQ> violation : violations) {
                com.vsc.legacy.model.availability.response.Error error = getAvailError(violation);
                if (error != null) {
    ...
    private com.vsc.legacy.model.availability.response.Error getAvailError(ConstraintViolation violation) {
        com.vsc.legacy.model.availability.response.Error error = null;
        if (!StringUtils.isEmpty(violation.getMessage())) {
            error = new com.vsc.legacy.model.availability.response.Error();
            error.setContent(violation.getMessage());
            String errorCode = violation.getMessageTemplate();
            errorCode = StringUtils.trimLeadingCharacter(errorCode,'{');
            errorCode = StringUtils.trimTrailingCharacter(errorCode, '}');
